...
class VacationGetAllForm(APIView):
    ...
    def get(self, request):
        ...
        serializer = VacationGetAllFormSerializer(
            queryset, many=True)  # Serialize the data
...
class VacationViewStaffForm(APIView):
    ...
    def get(self, request):
        ...
        queryset = get_all_objects.filter(
            user_id__employer=request.user.id).exclude(user_id=request.user.id)
        serializer = VacationSerializers(queryset, many=True)
        return Response(serializer.data)
