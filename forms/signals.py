...
from .models import StartUpsForm,ContactUs,PartnerMembership,InvestorRegistration,Entrepreuneur,ApplyJob  # Import the Vacation model from your app's models
...
def send_create_form_email(sender, instance, created, **kwargs):
    if created:
        ...
        subject = f"this is subject"
        from_email = "info@landaholding.com"  # Replace with your email address
        company_emails_list = ["relation@landaholding.com"]  # Replace with the recipient's email address


        send_mail(
        subject,
        'this is content of email ',  # Leave the message argument empty since we have HTML content.
        from_email,
        company_emails_list,
        fail_silently=False,
        ...
        send_mail(
        subject,
        'this is content of email customer',  # Leave the message argument empty since we have HTML content.
        from_email,
        user_list_email,
        fail_silently=False,
        # html_message=email_content,  # Specify the HTML content here.
